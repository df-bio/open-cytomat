...
class SerialPort:
    ...
    def issue_status_command(self, command: str) -> str:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                return self.check_prefix_and_strip(self.communicate(command), expected_prefix=expected_prefix)
            except UnexpectedResponse as exc:
                if attempt == max_attempts:
                    raise
                logger.warning(
                    "Unexpected response for status command %s (attempt %d/%d): %s; retrying",
                    command,
                    attempt,
                    max_attempts,
                    exc,
